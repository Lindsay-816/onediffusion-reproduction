# 此代码根据 https://lehduong.github.io/OneDiffusion-homepage/ 复现。
# 复现目的：完成领域工程创新实践课程实践要求。
# 原始来源：
#   - GitHub 仓库链接: https://github.com/lehduong/OneDiffusion
#   - 论文标题及作者: One Diffusion to Generate Them All by Duong H. Le
# 复现说明：该代码在部分文件路径以及环境配置上进行了修改。
# 注：本复现代码仅用于课程学习目的。
import os
import subprocess
import gradio as gr
...
NEGATIVE_PROMPT = ("monochrome, greyscale, low-res, bad anatomy, bad hands, text, error, "
                   "missing fingers, extra digit, fewer digits, cropped, worst quality, low quality,"
                   " normal quality, jpeg artifacts, signature, watermark, username, blurry, "
                   "artist name, poorly drawn, bad anatomy, wrong anatomy, extra limb, "
...
def initialize_models(captioner_name):
    ...
    # 构建模型路径 (相对于脚本位置)
    model_path = os.path.join(script_dir, "models", "OneDiffusion")
    # pipeline = OneDiffusionPipeline.from_pretrained("lehduong/OneDiffusion").to(device=device,
    # dtype=torch.bfloat16)
    pipeline = OneDiffusionPipeline.from_pretrained(
        "lehduong/OneDiffusion",
        cache_dir="/home/pengruilin/.cache/huggingface/hub"  # 显式指定Linux缓存路径
